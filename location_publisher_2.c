...
int main(void)
{
    ...
    *DDS_StringSeq_get_reference(&dp_qos.discovery.initial_peers, 0) = 
    ...
    dw_qos.protocol.rtps_object_id = k_OBJ_ID_PARTICIPANT02_DW01;
    ...
    while (1) {
        ...
        sample->obj_id = 2; 
        // add some arbitrary data to the sample
        sample->position.x = sin(sample_count/5);
        sample->position.y = cos(sample_count/5);
